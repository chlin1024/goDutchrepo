...
export async function getuserEmail(email: string) {
  const [userEmail]: any = await promisePool.query('Select email, id FROM users WHERE email = ?', [email]);
  return userEmail;
}

export async function getUserPassword(email: string) {
  const [userPassword]: any = await promisePool.query('Select password FROM users WHERE email = ?', [email]);
  return userPassword;
}

export async function getuserId(email: string) {
  const [result]: any = await promisePool.query('Select id FROM users WHERE email = ?', [email]);
...
export async function getuserData(email: string) {
  const [userId]: any = await promisePool.query('Select id FROM users WHERE email = ?', [email]);
  return userId;
}

export async function saveAccessTokenLine(accessTokenLine: string, userId: number) {
  const [result]: any = await promisePool.query('UPDATE users SET line_access_token = ? WHERE Id = ?', [
    accessTokenLine,
    userId,
  ]);
  return result;
...
export async function getAccessTokenLine(userId: number) {
  const [result]: any = await promisePool.query('SELECT line_access_token from users WHERE Id = ?', [userId]);
