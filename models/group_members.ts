...
export async function getGroupsByMemberId(userId: number) {
  const [groups]: any = await promisePool.query('SELECT group_id FROM group_members WHERE group_member_id = ?', [
    userId,
  ]);
  return groups;
}
...
export async function getGroupUsers(groupId: number) {
  const [results] = await promisePool.query('SELECT group_member_id FROM group_members WHERE group_id = ?;', [groupId]);
  return results;
}
